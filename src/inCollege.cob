...
01 connections-status          pic xx.
       88 connections-ok               value "00".
       88 connections-not-found        value "23".
       88 connections-file-missing     value "35".
01 conn-count                 pic 9(4) value 0.
01 conn-other                 pic x(30).
...
viewConnections.
       ...
       open input connections-database
       if connections-status = "00"
           perform until connections-status not = "00"
               read connections-database next record
                   at end
                       exit perform
                   not at end
                       if function trim(conn-user-1 trailing) = function trim(current-user trailing) or
                          function trim(conn-user-2 trailing) = function trim(current-user trailing)
                           if function trim(conn-user-1 trailing) = function trim(current-user trailing)
                               move conn-user-2 to conn-other
                           else
                               move conn-user-1 to conn-other
                           end-if
                           add 1 to conn-count
                           move spaces to output-buffer
                           string "- " delimited by size
                                  function trim(conn-other trailing) delimited by size
                                  into output-buffer
                           end-string
                           perform outputLine
                       end-if
